...
#         agg_match = re.search(r"SELECT\s+(SUM|AVG|MIN|MAX|COUNT)\((.*?)\)\s+AS\s+(\w+)", sql, re.IGNORECASE)
#         if agg_match:
#             agg_func = agg_match.group(1).upper()
#             agg_col = agg_match.group(2).strip()
#             agg_alias = agg_match.group(3)
#             agg_value = data[0][agg_alias] if data else 0

#             # Get number of records
#             where_clause = re.search(r"\bWHERE\b(.+?)(?:LIMIT|ORDER|$)", sql, re.IGNORECASE)
#             where_clause = where_clause.group(1).strip() if where_clause else ""
#             count_sql = f"SELECT COUNT(*) AS record_count FROM {table_name_upper}"
#             if where_clause:
#                 count_sql += f" WHERE {where_clause}"
#             count_result = execute_sql(count_sql)
#             record_count = count_result[0]["record_count"] if count_result else 0

#             # Generate aggregation-specific summary
#             col_desc = columns_meta.get(agg_col, {}).get("desc", agg_col) if metadata_available else agg_col
#             if agg_func == "SUM":
#                 summary.append(f"You have {record_count} records from {table_name_upper} whose {col_desc.lower()} totals {agg_value:.2f} {date_filter}")
#             elif agg_func == "AVG":
#                 summary.append(f"You have {record_count} records from {table_name_upper} with an average {col_desc.lower()} of {agg_value:.2f} {date_filter}")
#             elif agg_func == "MIN":
#                 summary.append(f"You have {record_count} records from {table_name_upper} with a minimum {col_desc.lower()} of {agg_value:.2f} {date_filter}")
#             elif agg_func == "MAX":
#                 summary.append(f"You have {record_count} records from {table_name_upper} with a maximum {col_desc.lower()} of {agg_value:.2f} {date_filter}")
#             elif agg_func == "COUNT":
#                 summary.append(f"You have {agg_value} records from {table_name_upper} {date_filter}")

#             # Add conditions
#             if business_terms or conditions:
#                 summary.append("matching the following conditions:")
#                 if business_terms:
#                     summary.append(f"- {', '.join(business_terms)}")
#                 for cond in conditions:
#                     cond_clean = re.sub(r"['\"]", "", cond).replace(" = ", " is ").replace(" > ", " exceeds ")
#                     summary.append(f"- {cond_clean}")

#             # Additional aggregations for numeric columns
#             if metadata_available and agg_col in columns_meta and columns_meta[agg_col].get("type") == "numeric":
#                 agg_sql_base = f"SELECT AVG({agg_col}) AS avg_val, MIN({agg_col}) AS min_val, MAX({agg_col}) AS max_val FROM {table_name_upper}"
#                 if where_clause:
#                     agg_sql_base += f" WHERE {where_clause}"
#                 agg_result = execute_sql(agg_sql_base)
#                 if agg_result:
#                     avg_val = agg_result[0]["avg_val"]
#                     min_val = agg_result[0]["min_val"]
#                     max_val = agg_result[0]["max_val"]
#                     summary.append("\nAdditional insights:")
#                     summary.append(f"- Average {col_desc.lower()}: {avg_val:.2f}")
#                     summary.append(f"- Minimum {col_desc.lower()}: {min_val:.2f}")
#                     summary.append(f"- Maximum {col_desc.lower()}: {max_val:.2f}")

#         else:
#             # Non-aggregation query
#             summary.append(f"You have {len(data)} records from {table_name_upper} {date_filter}")
...
#             if numeric_cols and metadata_available:
...
#                         col_desc = columns_meta.get(col, {}).get("desc", col)
#                         summary.append(f"- {col_desc}: Average = {avg:.2f}, Total = {total:.2f}, Min = {min_val}, Max = {max_val}")
...
#                     col_desc = columns_meta.get(col, {}).get("desc", col)
#                     summary.append(f"- {col_desc}: {percentage:.2f}% of records are true ({true_count} out of {len(data)})")
